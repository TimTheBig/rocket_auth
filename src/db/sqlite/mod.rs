...
impl DBConnection for Mutex<rusqlite::Connection> {
	...
	async fn update_user(&self, user: &User) -> Result<()> {
		let conn = self.lock().await;
		block_in_place(|| conn.execute(UPDATE_USER, params![user.id, user.email, user.password, user.is_admin]))?;
		Ok(())
	}
...
impl DBConnection for Mutex<SqliteConnection> {
	...
	async fn get_user_by_email(&self, email: &str) -> Result<User> {
		let mut db = self.lock().await;
		let user = query_as(SELECT_BY_EMAIL).bind(email).fetch_one(&mut *db).await?;
		Ok(user)
	}
}
#[cfg(feature = "sqlx-sqlite")]
